...
(function () {
  ...
  function renderChart(key,type){
    ...
    var pad=isRatio&&type==='bar'?{l:150,r:42,t:30,b:28}:{l:72,r:28,t:30,b:54};
    var values=data.map(function(d){return Math.max(0,Number(d.value)||0);}),max=Math.max.apply(null,values.concat([1])),min=0;
    if(isRatio&&type==='line'){min=Math.min.apply(null,values.concat([0]));max=Math.max.apply(null,values.concat([1]));if(max===min)max=min+1;}
    ...
    function tipShow(x,y,title,value){tip.innerHTML='<strong>'+esc(title)+'</strong><br>'+esc(value);tip.style.left=Math.max(12,Math.min((stage.clientWidth||w)-12,x))+'px';tip.style.top=Math.max(42,Math.min((stage.clientHeight||h)-12,y))+'px';tip.classList.add('show');}
    ...
    if(type==='line'){
      grid(); var pw=w-pad.l-pad.r,ph=h-pad.t-pad.b,pts=data.map(function(d,i){return{x:pad.l+(data.length===1?pw/2:i*pw/(data.length-1)),y:pad.t+ph-((Number(d.value)-min)/(max-min||1))*ph};});
      svg.appendChild(svgEl('polygon',{class:'pp-chart-area',points:pts.map(function(p){return p.x+','+p.y;}).join(' ')+' '+(w-pad.r)+','+(h-pad.b)+' '+pad.l+','+(h-pad.b)}));
      var path=svgEl('path',{class:'pp-chart-line',d:pts.map(function(p,i){return(i?'L':'M')+p.x.toFixed(1)+' '+p.y.toFixed(1);}).join(' ')});svg.appendChild(path);
      pts.forEach(function(p,i){var c=svgEl('circle',{class:'pp-chart-point',cx:p.x,cy:p.y,r:5});c.addEventListener('mouseenter',function(){tipShow(p.x,p.y,data[i].label,fmt(data[i].value));});c.addEventListener('mouseleave',tipHide);svg.appendChild(c);var tx=svgEl('text',{class:'pp-chart-axis',x:p.x,y:h-16,'text-anchor':'middle'});tx.textContent=data[i].label;svg.appendChild(tx);});
    }else if(type==='bar'){
      var horizontal=isRatio&&data.length>=7;
      if(horizontal){var left=pad.l,top=pad.t,plotW=w-pad.l-pad.r,rowH=(h-pad.t-pad.b)/data.length;data.forEach(function(d,i){var y=top+i*rowH+rowH*.18,bh=rowH*.64,bw=(Number(d.value)||0)/(max||1)*plotW,label=svgEl('text',{class:'pp-chart-axis',x:left-12,y:y+bh*.68,'text-anchor':'end'});label.textContent=d.label;svg.appendChild(label);var r=svgEl('rect',{class:'pp-chart-bar',x:left,y:y,width:Math.max(2,bw),height:bh,rx:7});r.style.animationDelay=(i*.045)+'s';r.addEventListener('mouseenter',function(){tipShow(left+bw,y,d.label,fmt(d.value));});r.addEventListener('mouseleave',tipHide);svg.appendChild(r);});}
      else{grid();var pw2=w-pad.l-pad.r,ph2=h-pad.t-pad.b,groupW=pw2/data.length,bw2=Math.min(56,groupW*.62);data.forEach(function(d,i){var v=Math.max(0,Number(d.value)||0),bh=ph2*(v/max),x=pad.l+i*groupW+(groupW-bw2)/2,y=h-pad.b-bh,r2=svgEl('rect',{class:'pp-chart-bar',x:x,y:y,width:bw2,height:Math.max(2,bh),rx:8});r2.style.animationDelay=(i*.045)+'s';r2.addEventListener('mouseenter',function(){tipShow(x+bw2/2,y,d.label,fmt(d.value));});r2.addEventListener('mouseleave',tipHide);svg.appendChild(r2);var tx=svgEl('text',{class:'pp-chart-axis',x:x+bw2/2,y:h-16,'text-anchor':'middle'});tx.textContent=d.label;svg.appendChild(tx);});}
    }else{
      var total=values.reduce(function(a,b){return a+b;},0)||1,cx=w/2,cy=h/2,r=Math.min(110,(h-40)/2),circ=2*Math.PI*r,offset=0;
      data.forEach(function(d,i){var dash=values[i]/total*circ,c=svgEl('circle',{class:'pp-chart-pie',cx:cx,cy:cy,r:r,fill:'none',stroke:colors[i%colors.length],'stroke-width':40,'stroke-dasharray':dash+' '+circ,'stroke-dashoffset':-offset});c.style.transformOrigin=cx+'px '+cy+'px';c.style.animationDelay=(i*.05)+'s';c.addEventListener('mouseenter',function(){tipShow(cx,cy-r,d.label,fmt(d.value)+' · '+(values[i]/total*100).toFixed(1)+'%');});c.addEventListener('mouseleave',tipHide);svg.appendChild(c);offset+=dash;});
      svg.appendChild(svgEl('circle',{cx:cx,cy:cy,r:r-22,fill:'var(--card-bg,var(--card))'}));var tv=svgEl('text',{class:'pp-chart-center',x:cx,y:cy-2});tv.textContent=fmt(total);svg.appendChild(tv);var ts=svgEl('text',{class:'pp-chart-center-sub',x:cx,y:cy+18});ts.textContent='Total';svg.appendChild(ts);
    }
    stage.appendChild(svg); data.forEach(function(d,i){var dot=document.createElement('span');dot.innerHTML='<i class="pp-chart-dot" style="background:'+(type==='pie'?colors[i%colors.length]:'var(--gold)')+'"></i>'+esc(d.label);legend.appendChild(dot);});
  }

  function showChart(key,title,desc,data,format){chartState[key]={data:data,format:format,colors:['#c18a27','#2bb8a8','#7f8ce8','#a88cf2','#e6bf69','#49cf78','#ee6f7f','#5d9bf0']};var panel=chartPanel(key,title,desc);renderChart(key,panel.querySelector('.pp-fin-chart__tab.active').getAttribute('data-chart'));}
  ...
  function calculateSIP(){
    ...
    showChart('sip','SIP Growth & Wealth Journey','Explore your yearly accumulated value with interactive line, bar and pie views.',(hasStep?stepRows:regularRows).map(function(r){return{label:'Y'+r.year,value:r.value};}),function(v){return inr(v);});
  }
